...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='W:\\Python\\Psychopy\\untitled.py',
...
feedbacktxt = visual.TextStim(win=win, name='feedbacktxt',
    text=' You took  ' + str(code.outertrials[-1]) +' trials. \n Time take for memorization was: ' + str(sum(response.rt)),
    font='Arial',
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if image.status == STARTED:
        ...
        if tThisFlipGlobal > image.tStartRefresh + 7.0-frameTolerance:
    ...
    for thisExptrial in exptrials:
        ...
        while continueRoutine:
            ...
            if response.status == STARTED and not waitOnFlip:
                theseKeys = response.getKeys(keyList=['up', 'down', 'left', 'right'], waitRelease=False)
...
continueRoutine = True
routineTimer.add(8.100000)
# update component parameters for each repeat
# keep track of which components have finished
feedbackComponents = [feedbacktxt, thanktxt]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    # *feedbacktxt* updates
    if feedbacktxt.status == NOT_STARTED and tThisFlip >= 0.05-frameTolerance:
        # keep track of start time/frame for later
    ...
    if feedbacktxt.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > feedbacktxt.tStartRefresh + 3-frameTolerance:
            # keep track of stop time/frame for later
    ...
    # *thanktxt* updates
    if thanktxt.status == NOT_STARTED and tThisFlip >= 3.1-frameTolerance:
        # keep track of start time/frame for later
